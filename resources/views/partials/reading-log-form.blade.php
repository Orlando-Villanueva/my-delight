...
<div class="max-w-2xl mx-auto p-6">
    <div class="flex items-center justify-between mb-8">
        ...
        {{-- Cancel Button - Returns to Dashboard via HTMX --}}
    ...
    <form hx-post="{{ route('logs.store') }}" 
        ...
        <div class="space-y-2">
            ...
            <textarea id="notes_text" 
                      name="notes_text" 
                      rows="4" 
                      maxlength="500"
                      x-model="form.notes_text"
...
function readingLogForm() {
    return {
        form: {
            book_id: '{{ old('book_id', '') }}',
            book_name: '',
            chapter_input: '{{ old('chapter_input', '') }}',
            date_read: '{{ old('date_read', today()->toDateString()) }}',
            notes_text: `{{ old('notes_text', '') }}`,
            chapter_validation_message: '',
            chapter_validation_valid: false
        },
        availableChapters: 0,
        characterCount: {{ old('notes_text') ? strlen(old('notes_text')) : 0 }},
        ...
        validateChapterInput() {
            ...
            if (rangeMatch) {
                ...
                if (start > end) {
                    this.form.chapter_validation_message = '❌ Start chapter must be less than or equal to end chapter';
                    this.form.chapter_validation_valid = false;
                    return;
                }
                
                if (start < 1 || end > this.availableChapters) {
                    this.form.chapter_validation_message = `❌ Chapters must be between 1 and ${this.availableChapters}`;
                    this.form.chapter_validation_valid = false;
                    return;
                }
                
                const chapterCount = end - start + 1;
                this.form.chapter_validation_message = `✅ ${chapterCount} chapters (${this.form.book_name} ${start}-${end})`;
            ...
            if (singleMatch) {
                ...
                if (chapter < 1 || chapter > this.availableChapters) {
                    this.form.chapter_validation_message = `❌ Chapter must be between 1 and ${this.availableChapters}`;
                    this.form.chapter_validation_valid = false;
                    return;
                }
